...
from transformers import AutoConfig, AutoModel, AutoModelForCausalLM,VisionEncoderDecoderConfig
...
class VisionEncoderDecoderModel(PreTrainedModel):
    ...
    def __init__(
        ...
        if encoder is None:
            encoder = AutoModel.from_pretrained(config.vision_name,config=config.encoder)

        if decoder is None:
            decoder = AutoModelForCausalLM.from_pretrained(config.lm_name,config=config.decoder)
